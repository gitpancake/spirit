...
/**
 * @title FixedPriceManifestoMinter (ERC-1155)
 ...
 *
 * CROSSMINT COMPATIBLE:
 * - Standard mint() and mintTo() interfaces
 * - Can integrate credit card payments (Crossmint converts to USDC)
 */
contract FixedPriceManifestoMinter is Ownable, ReentrancyGuard, Pausable {
    ...
    /// @notice Tracking if a manifesto has been distributed to subscribers (by manifestoId)
    mapping(uint256 => bool) public manifestoDistributed;
    ...
    error InvalidAmount();
    error EmptyBatch();
    error ManifestoAlreadyDistributed(uint256 manifestoId);

    // ============ Constructor ============

    /**
     * @notice Initialize the Fixed Price Manifesto Minter (ERC-1155)
    ...
    /**
     * @notice Check if a manifesto has been distributed to subscribers
     * @param manifestoId Manifesto ID to check
     * @return True if this manifesto has been distributed
     */
    function distributed(uint256 manifestoId) external view returns (bool) {
        return manifestoDistributed[manifestoId];
    }
    ...
    function distributeToSubscribersBatch(
        ...
        // Prevent duplicate distribution to same manifesto
        // If you need to distribute more copies, use adminMintBatch()
        if (manifestoDistributed[manifestoId]) {
            revert ManifestoAlreadyDistributed(manifestoId);
        }
        ...
        // Update total minted counter and mark as distributed
        if (successCount > 0) {
            unchecked {
                totalMinted += successCount;
            }

            // Mark manifesto as distributed if at least one successful mint
            manifestoDistributed[manifestoId] = true;
        }
